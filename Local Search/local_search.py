...
def simulatedAnnealing(initBoard, decayRate, T_Threshold):
    global heuristic_total
    T = 100
    current = initBoard
    initHeuristic = numAttackingQueens(current)
    print("H-Value:", initHeuristic)
    heuristic_total += initHeuristic
    while T > T_Threshold:
        randomSelection = random.choice(getSuccessorStates(current))
        if (numAttackingQueens(randomSelection) < numAttackingQueens(current)):
            current = randomSelection
        else:
            change = numAttackingQueens(current) - numAttackingQueens(randomSelection)
            value = math.exp(change / T)
            if random.random() < value:
                current = randomSelection
            T = scheduling(T, decayRate)
    print("Final board with h value:")
    current.printBoard()
    print()
...
def main():
    ...
    for i in range(10):
        ...
        board.printBoard()
        print()
        simulatedAnnealing(board, 0.8, 0.00001)
    print("-------------------------")
    print("Average H-cost:", heuristic_total / 10)
    print("-------------------------")
    print("Board Size: 4")
    print("-------------------------")
    print("\n")
